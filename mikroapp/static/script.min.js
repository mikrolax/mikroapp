...
$spy.affix(data)})})}(window.jQuery);(function(window,undefined){function _applyAttrs(context,attrs){for(var attr in attrs){if(attrs.hasOwnProperty(attr)){context[attr]=attrs[attr];}}}
...
t=b+p+w;return t;}
function _outerHeight(el){var b=parseInt(_getStyle(el,'border-top-width'),10)+parseInt(_getStyle(el,'border-bottom-width'),10),p=parseInt(_getStyle(el,'padding-top'),10)+parseInt(_getStyle(el,'padding-bottom'),10),w=el.offsetHeight,t;if(isNaN(b)){b=0;}
t=b+p+w;return t;}
...
return theText;}
function _setText(el,content){if(typeof document.body.innerText=='string'){content=content.replace(/ /g,'\u00a0');el.innerText=content;}
else{content=content.replace(/</g,'&lt;');content=content.replace(/>/g,'&gt;');content=content.replace(/\n/g,'<br>');content=content.replace(/\s\s/g,' &nbsp;')
...
self.preview();self.editorIframeDocument.body.focus();self.emit('fullscreenenter');};self._exitFullscreen=function(el){_saveStyleState(self.element,'apply',_elementStates.element);_saveStyleState(self.iframeElement,'apply',_elementStates.iframeElement);_saveStyleState(self.editorIframe,'apply',_elementStates.editorIframe);_saveStyleState(self.previewerIframe,'apply',_elementStates.previewerIframe);self.element.style.width=self._eeState.reflowWidth?self._eeState.reflowWidth:'';self.element.style.height=self._eeState.reflowHeight?self._eeState.reflowHeight:'';utilBtns.style.visibility='visible';if(!nativeFs){document.body.style.overflow='auto';}
else{document.webkitCancelFullScreen();}
self._eeState.fullscreen=false;if(_isInEdit){self.edit();}
else{self.preview();}
...
else if(targetClass.indexOf('epiceditor-fullscreen-btn')>-1){self._goFullscreen(fsElement);}});if(document.body.webkitRequestFullScreen){fsElement.addEventListener('webkitfullscreenchange',function(){if(!document.webkitIsFullScreen){self._exitFullscreen(fsElement);}},false);}
utilBar=self.iframe.getElementById('epiceditor-utilbar');utilBar.style.display='none';utilBar.addEventListener('mouseover',function(){if(utilBarTimer){clearTimeout(utilBarTimer);}});function utilBarHandler(e){if(Math.abs(mousePos.y-e.pageY)>=5||Math.abs(mousePos.x-e.pageX)>=5){utilBar.style.display='block';if(utilBarTimer){clearTimeout(utilBarTimer);}
...
if(isMod===true&&e.keyCode==self.settings.shortcut.preview&&!self.is('fullscreen')){e.preventDefault();if(self.is('edit')){self.preview();}
else{self.edit();}}
if(isMod===true&&e.keyCode==self.settings.shortcut.fullscreen){e.preventDefault();self._goFullscreen(fsElement);}
...
self.iframe.close();callback.call(this);this.emit('load');return this;}
EpicEditor.prototype.unload=function(callback){if(this.is('unloaded')){throw new Error('Editor isn\'t loaded');}
var self=this,editor=window.parent.document.getElementById(self._instanceId);editor.parentNode.removeChild(editor);self._eeState.loaded=false;self._eeState.unloaded=true;callback=callback||function(){};if(self.saveInterval){window.clearInterval(self.saveInterval);}
callback.call(this);self.emit('unload');return self;}
EpicEditor.prototype.reflow=function(kind){var self=this,widthDiff=_outerWidth(self.element)-self.element.offsetWidth,heightDiff=_outerHeight(self.element)-self.element.offsetHeight,elements=[self.iframeElement,self.editorIframe,self.previewerIframe],newWidth,newHeight;for(var x=0;x<elements.length;x++){if(!kind||kind=='width'){newWidth=self.element.offsetWidth-widthDiff+'px';elements[x].style.width=newWidth;self._eeState.reflowWidth=newWidth;}
...
return self;}
EpicEditor.prototype.preview=function(theme){var self=this,x,anchors;theme=theme||self.settings.basePath+self.settings.theme.preview;_replaceClass(self.getElement('wrapper'),'epiceditor-edit-mode','epiceditor-preview-mode');if(!self.previewerIframeDocument.getElementById('theme')){_insertCSSLink(theme,self.previewerIframeDocument,'theme');}
...
self.previewer.innerHTML=self.exportFile(null,'html');anchors=self.previewer.getElementsByTagName('a');for(x in anchors){if(anchors[x].hash&&anchors[x].hostname==window.location.hostname){anchors[x].target='_self';}}
if(!self.is('fullscreen')){self.editorIframe.style.display='none';self.previewerIframe.style.display='block';self._eeState.preview=true;self._eeState.edit=false;self.previewerIframe.focus();}
...
EpicEditor.prototype.edit=function(){var self=this;_replaceClass(self.getElement('wrapper'),'epiceditor-preview-mode','epiceditor-edit-mode');self._eeState.preview=false;self._eeState.edit=true;self.editorIframe.style.display='block';self.previewerIframe.style.display='none';self.editorIframe.focus();self.emit('edit');return this;}
...
return this;}
EpicEditor.prototype.save=function(){var self=this,storage,isUpdate=false,file=self.settings.file.name,content=_getText(this.editor);this._canSave=true;storage=JSON.parse(this._storage[self.settings.localStorageName]);if(storage[file]===undefined){storage[file]=self._defaultFileSchema();}
else if(content!==storage[file].content){storage[file].modified=new Date();isUpdate=true;}
storage[file].content=content;this._storage[self.settings.localStorageName]=JSON.stringify(storage);if(isUpdate){self.emit('update');}
this.emit('save');return this;}
...
return this;};EpicEditor.prototype.exportFile=function(name,kind){var self=this,file,content;name=name||self.settings.file.name;kind=kind||'text';file=self.getFiles(name);if(file===undefined){return;}
content=file.content;switch(kind){case'html':content=content.replace(/\u00a0/g,' ').replace(/&nbsp;/g,' ');return self.settings.parser(content);case'text':content=content.replace(/\u00a0/g,' ').replace(/&nbsp;/g,' ');return content;default:return content;}}
EpicEditor.prototype.getFiles=function(name){var files=JSON.parse(this._storage[this.settings.localStorageName]);if(name){return files[name];}
else{return files;}}
...
EpicEditor.version='0.2.0';EpicEditor._data={};window.EpicEditor=EpicEditor;})(window);;(function(){var block={newline:/^\n+/,code:/^( {4}[^\n]+\n*)+/,fences:noop,hr:/^( *[-*_]){3,} *(?:\n+|$)/,heading:/^ *(#{1,6}) *([^\n]+?) *#* *(?:\n+|$)/,lheading:/^([^\n]+)\n *(=|-){3,} *\n*/,blockquote:/^( *>[^\n]+(\n[^\n]+)*\n*)+/,list:/^( *)(bull) [^\0]+?(?:hr|\n{2,}(?! )(?!\1bull )\n*|\s*$)/,html:/^ *(?:comment|closed|closing) *(?:\n{2,}|\s*$)/,def:/^ *\[([^\]]+)\]: *([^\s]+)(?: +["(]([^\n]+)[")])? *(?:\n+|$)/,paragraph:/^([^\n]+\n?(?!body))+\n*/,text:/^[^\n]+/};block.bullet=/(?:[*+-]|\d+\.)/;block.item=/^( *)(bull) [^\n]*(?:\n(?!\1bull )[^\n]*)*/;block.item=replace(block.item,'gm')
(/bull/g,block.bullet)
